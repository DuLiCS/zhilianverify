...
# ret,grey_img = cv2.threshold(gray_img,10,255,cv2.THRESH_BINARY)

grey_img= cv2.adaptiveThreshold(gray_img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,5,10)
...
cv2.imshow('thresh',rearrange_img)


vcode = pytesseract.image_to_string(veri,lang="chi_sim",config="-psm 8")


cv2.imwrite('33.tif',rearrange_img)
